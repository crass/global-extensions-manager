...
var AddonRepository = {
  ...
  // Info on which app to search as
  _appinfo: Services.appinfo,
      ...
      switch (localName) {
        ...
        case "all_compatible_os":
          ...
          addon.isPlatformCompatible = Array.some(nodes, function(aNode) {
            ...
            return text == "all" || text == self._appinfo.OS.toLowerCase();
          });
          break;
        case "install":
          ...
          if (node.hasAttribute("os")) {
            ...
            if (os != "all" && os != self._appinfo.OS.toLowerCase())
              break;
          }
  ...
  _parseAddons: function(aElements, aTotalResults, aSkip) {
    ...
    for (let i = 0; i < aElements.length && results.length < this._maxResults; i++) {
      ...
      let compatible = Array.some(applications, function(aAppNode) {
        if (self._getTextContent(aAppNode) != self._appinfo.ID)
          return false;
        ...
        let currentVersion = self._appinfo.version;
